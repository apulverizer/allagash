...
class TestProblem:
    ...
    def test_init_invalid_problem_type_option(
        self, binary_lscp_pulp_problem, binary_coverage
    ):
        with pytest.raises(ValueError) as e:
            Problem(binary_lscp_pulp_problem, binary_coverage, "test")
        assert e.value.args[0] == "Invalid problem_type: 'test'"
    ...
    def test_selected_supply_list(self, mclp_problem_solved):
        assert isinstance(
            mclp_problem_solved.selected_supply(mclp_problem_solved.coverages[0]), list
        )
    ...
    def test_selected_supply_invalid_coverage(self, mclp_problem_solved):
        ...
        assert (
            e.value.args[0]
            == "Expected 'Coverage' type for coverage, got '<class 'NoneType'>'"
        )

    def test_selected_supply_invalid_operator(self, mclp_problem_solved):
        with pytest.raises(TypeError) as e:
            mclp_problem_solved.selected_supply(
                mclp_problem_solved.coverages[0], operation=None
            )
        assert (
            e.value.args[0]
            == "Expected callable for operation, got '<class 'NoneType'>'"
        )

    def test_selected_supply_invalid_value(self, mclp_problem_solved):
        with pytest.raises(TypeError) as e:
            mclp_problem_solved.selected_supply(
                mclp_problem_solved.coverages[0], value=None
            )
        assert (
            e.value.args[0]
            == "Expected 'int' or 'float' for value, got '<class 'NoneType'>'"
        )

    def test_selected_demand(self, mclp_problem_solved):
        assert isinstance(
            mclp_problem_solved.selected_demand(mclp_problem_solved.coverages[0]), list
        )
...
class TestLSCPProblem:
    ...
    def test_lscp_invalid_coverages2(self, binary_coverage, partial_coverage):
        with pytest.raises(ValueError) as e:
            Problem.lscp([binary_coverage, partial_coverage])
        assert (
            e.value.args[0]
...
class TestMCLPProblem:
    ...
    def test_mclp_invalid_coverages2(self, binary_coverage, partial_coverage):
        with pytest.raises(ValueError) as e:
            Problem.mclp(
                [binary_coverage, partial_coverage],
                max_supply={binary_coverage: 5, partial_coverage: 5},
            )
        assert (
            e.value.args[0]
            == "Invalid coverages. Coverages must have the same coverage type."
    ...
    def test_mclp_invalid_coverages3(self, partial_coverage):
        with pytest.raises(ValueError) as e:
            Problem.mclp(partial_coverage, max_supply={partial_coverage: 5})
    ...
    def test_mclp_invalid_coverages4(self, binary_coverage):
        with pytest.raises(TypeError) as e:
            Problem.mclp(binary_coverage, max_supply={binary_coverage: 5.5})
        assert (
            e.value.args[0]
    ...
    def test_mclp_invalid_coverages5(self, binary_coverage):
        with pytest.raises(TypeError) as e:
            Problem.mclp(binary_coverage, max_supply={"test": 5})
        assert (
            e.value.args[0]
    ...
    def test_mclp_invalid_coverages6(self, binary_coverage):
        with pytest.raises(TypeError) as e:
            Problem.mclp(binary_coverage, max_supply=None)
        assert (
            e.value.args[0]
...
class TestBCLPProblem:
    def test_bclp(self, binary_coverage, binary_coverage2):
        ...
        assert p.problem_type == "bclp"

    def test_bclp_invalid_coverages(self):
        with pytest.raises(TypeError) as e:
            Problem.bclp(None, max_supply={})
        assert (
            e.value.args[0]
            == "Expected 'Coverage' or 'list' type for coverages, got '<class 'NoneType'>'"
        )

    def test_bclp_invalid_coverages2(self, binary_coverage, partial_coverage):
        with pytest.raises(ValueError) as e:
            Problem.bclp(
                [binary_coverage, partial_coverage],
                max_supply={binary_coverage: 5, partial_coverage: 5},
    ...
    def test_bclp_invalid_coverages3(self, partial_coverage):
        with pytest.raises(ValueError) as e:
            Problem.bclp(partial_coverage, max_supply={partial_coverage: 5})
        assert e.value.args[0] == "BCLP can only be generated from binary coverage."

    def test_bclp_invalid_coverages4(self, binary_coverage):
        with pytest.raises(TypeError) as e:
            Problem.bclp(binary_coverage, max_supply={binary_coverage: 5.5})
        assert (
            e.value.args[0]
            == "Expected 'int' type as value in max_supply, got '<class 'float'>'"
        )

    def test_bclp_invalid_coverages5(self, binary_coverage):
        with pytest.raises(TypeError) as e:
            Problem.bclp(binary_coverage, max_supply={"test": 5})
        assert (
            e.value.args[0]
            == "Expected 'Coverage' type as key in max_supply, got '<class 'str'>'"
        )

    def test_bclp_invalid_coverages6(self, binary_coverage):
        with pytest.raises(TypeError) as e:
            Problem.bclp(binary_coverage, max_supply=None)
        assert (
            e.value.args[0]
            == "Expected 'dict' type for max_supply, got '<class 'NoneType'>'"
        )

    def test_bclp_invalid_coverages7(self, binary_coverage_no_demand):
        with pytest.raises(TypeError) as e:
            Problem.bclp(
                binary_coverage_no_demand, max_supply={binary_coverage_no_demand: 5}
            )

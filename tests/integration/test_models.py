...
import pytest
import math
from allagash.model import Model, InfeasibleException, UndefinedException
...
class TestMCLP:
    ...
    def test_single_supply(self, demand_points_dataframe, binary_coverage):
        ...
        res = demand_points_dataframe.query(f"GEOID10 in ({[f'{i}' for i in  model.selected_demand(binary_coverage)]})")
...
class TestLSCP:
    ...
    def test_multiple_supply(self, demand_points_dataframe, binary_coverage, binary_coverage2):
        ...
        model.solve(pulp.GLPK())
        selected_locations = model.selected_supply(binary_coverage)
        selected_locations2 = model.selected_supply(binary_coverage2)
        covered_demand = demand_points_dataframe.query(
            f"GEOID10 in ({[f'{i}' for i in model.selected_demand(binary_coverage)]})")
